...
#define DIST_MIN 60
#define DIST_MAX 100
#define TIMEOUT 5000 // timeout in ms (NEW)
...
void camcar(int argc, char *argv[], struct thread_dat *ptdat) 
{
    ...
    while (ch != 'q') {
        ...
        int speed = 20; // fwd + bwd speed of the car (NEW)
        int turnSpeed = 20; // turning speed of the car (NEW)
        unsigned long searchStartTime = 0; // start time of the blob search (NEW)
        int timeoutActive = 0; // flag to indicate that a timeout is active (NEW)
        double alignThreshold = 0.15; // threshold for aligning the car to the blob (NEW)
        int blobThreshold = 20; // threshold for blob size (NEW)
        ...
        if (obstacle) {
            ...
            // TODO: thread communication: copy blob from camera thread (mutex protected) (DONE)
            pthread_mutex_lock(&count_mutex);// locking the mutex
            blob = ptdat->blob;// copying the blob data
            blobnr = ptdat->blobnr;// update blobnr
            ...
            blobSufficient = (blob.size > blobThreshold);  // TODO: experiment with that threshold value
            ...
            if ( ! blobSufficient ) {
                ...
                if (blobnr < ptdat->blobnr) {
                    blobnr = ptdat->blobnr;
                    timeoutActive = 0; //resetting the timeout
                    // TODO: potential actions: turn car or camera platform a few steps around and see if a blob is to be found (DONE)   
                }
                else
                {
                ...
                if (!carBlobAligned) {
                    ...
                    if (blobnr < ptdat->blobnr) {
                        if (blob.halign < 0) {
                            initio_SpinRight(turnSpeed); //(Testing between 20 and 40) 
                            // TODO: slightly turn car to align with blob (find a useful turn duration)
                            //       if blob.halign is negative, then turn right, otherwise left (DONE)
                            blobnr = ptdat->blobnr;
                            delay(100);
                        }
                        else {
                            initio_SpinLeft(turnSpeed);
                            blobnr = ptdat->blobnr;
                            delay(100);
                        }
                } else {
                    ...
                    case toofar:
                        ...
                        clrtoeol(); // curses library
                        // TODO: move car forward to come closer (DONE)
                        initio_DriveForward(speed);
                        break;
                    case tooclose:
                        mvprintw(3, 1,"State RB (drive backwards), dist=%d", distance);
                        clrtoeol(); // curses library
                        // TODO: move car backwards to get more distance (DONE)
...
void *worker(void *p_thread_dat)
{
  ...
  while (ptdat->bExit == 0) {
    ...
    // TODO: fill in code: copy blob into shared data (mutex protected) (DONE)
...
int main (int argc, char *argv[])
{
  ...
  // TODO: create thread via pthread_create() (DONE)
  ...
  // TODO: wait for thread to finish via pthread_join(): (DONE)
  if (pthread_join(cam_thread, NULL) != 0) {
  ...
  // TODO: distroy thread attribute pt_attr via pthread_attr_destroy() (DONE)
